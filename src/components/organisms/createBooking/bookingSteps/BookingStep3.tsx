...
import type { BookingPro, BookingFormValues } from '../../../../types/booking'
  ...
  return (
    <>
      ...
      <div className={`${gfx.card} p-7 mb-4`}>
        ...
        {rows.map(({ label, value, mono, price }) => (
          ...
          >
            ...
            <span
              className={`font-semibold text-ink ${mono ? 'font-mono' : ''} ${price ? 'text-ok font-mono' : ''}`}
            >
              {value || '—'}
            </span>
          </div>
      ...
      <div className={`${gfx.card} p-5 mb-4`}>
        ...
        <p className="text-[14.5px] text-ink-soft leading-relaxed m-0">
      ...
      {image && (
        <div className={`${gfx.card} p-5`}>
          <div className="text-[13px] font-semibold text-ink-soft mb-3">
            {t('booking_summary_photos') ?? 'Photo'}
          </div>
          <img
            src={image.uri}
            alt=""
