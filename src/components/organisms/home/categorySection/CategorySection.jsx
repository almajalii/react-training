...
export default function CategorySection() {
  const { t, categories, isLoading, goToCategory } = useCategorySection();
    ...
    <section className="bg-page pt-10 pb-20 px-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-12">
          ...
          <h2 className="text-4xl font-extrabold text-ink tracking-tight leading-tight">{t('category_heading')}</h2>
        </div>

        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className={`${gfx.card} p-7 min-h-45 animate-pulse`}>
                <div className="w-11 h-11 rounded-xl bg-chip mb-4" />
                <div className="h-4 bg-chip rounded w-2/3 mb-2" />
                <div className="h-3 bg-chip rounded w-full" />
              </div>
            ))}
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {categories.map(({ id, slug, Icon, displayName, description }) => (
              <button
                key={id}
              ...
              >
                ...
                <div className="flex-1">
                  <p className="font-bold text-ink text-[17px] tracking-[-0.01em]">{displayName}</p>
                  {description && (
                    <p className="text-muted text-[13px] leading-relaxed mt-1.5 line-clamp-2">{description}</p>
                  )}
                </div>
